...
17. **Boundary State Machine (model = facts, controller = routing)** — The MCP server exposes leaf facts and the controller (session.md) decides flow. Lesson completion (`complete_lesson`) is separate from moving the pointer (`advance_position`): the pointer stays on a chapter/part's final lesson until its boundary work is *resolved*, so a restart at a boundary re-offers that work instead of skipping it. `get_current_position` returns boundary facts (`is_chapter_end`, `is_part_end`, `interview_resolved`, `capstone_resolved`); session.md's "Lesson Boundary Routing" applies them in order (interview → capstone → advance), and `advance_position` is guarded to refuse moving off an incomplete lesson or an unresolved boundary. "Resolved" = a result row exists (the work was done, or — capstone only, since it's skippable — explicitly skipped). Chapter/part completion is **derived** on read, never stored (a stored flag drifted): a chapter is complete when its lessons are done and its interview resolved; a part is complete when its chapters are — the capstone is an optional ★/☆ bonus, **not** a completion gate. Atomic facts are stored; aggregates are computed.
