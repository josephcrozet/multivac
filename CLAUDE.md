...
17. **Boundary State Machine (model = facts, controller = routing)** — The MCP server exposes leaf facts and the controller (session.md) decides flow. Lesson completion (`complete_lesson`) is separate from moving the pointer (`advance_position`): the pointer stays on a chapter/part's final lesson until its Mock Interview / Capstone is logged, so a restart at a boundary re-offers that work instead of skipping it. `get_current_position` returns boundary facts (`is_chapter_end`, `is_part_end`, `interview_logged`, `capstone_logged`); session.md's "Lesson Boundary Routing" applies them in order (interview → capstone → advance). Chapter/part completion is **derived** on read (lessons + interview/capstone logs), never stored — a stored flag was a denormalized cache that drifted. Atomic facts (lesson completion, interview/capstone logs) are stored; aggregates are computed.
...
| `get_current_position` | Current lesson/chapter/part, type, difficulty_level, and boundary facts (is_chapter_start/is_chapter_end/is_part_end/interview_logged/capstone_logged) |
